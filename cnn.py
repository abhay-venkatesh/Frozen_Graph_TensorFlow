...
class CNN(object):
    ...
    def network(self, input, dropout_rate):
        ...
        pool1 = tf.layers.max_pooling2d(
            inputs=conv2, pool_size=[2, 2], strides=[2, 2], name="pool1"
        )

        pool1_dropout = tf.layers.dropout(
            inputs=pool1, rate=dropout_rate, training=True, name="pool1_dropout"
        )
        ...
        pool2 = tf.layers.max_pooling2d(
            inputs=conv4, pool_size=[2, 2], strides=[2, 2], name="pool2"
        )

        pool2_dropout = tf.layers.dropout(
            inputs=pool2, rate=dropout_rate, training=True, name="pool2_dropout"
        )
        ...
        pool3 = tf.layers.max_pooling2d(
            inputs=conv5, pool_size=[2, 2], strides=[2, 2], name="pool3"
        )

        pool3_dropout = tf.layers.dropout(
            inputs=pool3, rate=dropout_rate, training=True, name="pool3_dropout"
        )

        flat = tf.layers.flatten(inputs=pool3_dropout, name="flat")

        fc1 = tf.layers.dense(inputs=flat, units=256, activation=tf.nn.relu, name="fc1")

        fc1_dropout = tf.layers.dropout(
            inputs=fc1, rate=dropout_rate, training=True, name="fc1_dropout"
        )

        fc2 = tf.layers.dense(
            inputs=fc1_dropout, units=self.num_classes, activation=None, name="fc2"
        )
    ...
    def validate(self, data, label):
        ...
        output, validate_loss = self.sess.run(
    ...
    def test(self, data):
        ...
        output = self.sess.run(
            self.output, feed_dict={self.input: data, self.dropout_rate: 0.0}
        )
    ...
    def save_signature(self, directory):
        ...
        signature = signature_def_utils.build_signature_def(
            inputs={
                "input": saved_model_utils.build_tensor_info(self.input),
                "dropout_rate": saved_model_utils.build_tensor_info(self.dropout_rate),
            },
