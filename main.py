...
def train(
    ...
    for epoch in range(epochs):
        ...
        for i, idx in enumerate(mini_batch_idx):
            ...
            if i % 200 == 0:
                ...
                train_prediction = np.argmax(train_prediction_onehot, axis=1).reshape(
                    (-1, 1)
                )
                train_accuracy = model_accuracy(
                    label=y_train[idx], prediction=train_prediction
                )
                print(
    ...
    plot_curve(
        ...
        train_accuracies=train_accuracy_log,
        valid_accuracies=valid_accuracy_log,
...
def main():
    ...
    parser.add_argument(
        "--lr", type=float, help="initial learning rate", default=learning_rate_default
    )
    parser.add_argument(
        "--lr_decay",
        type=float,
    ...
    parser.add_argument(
        "--dropout", type=float, help="dropout rate", default=dropout_rate_default
    )
    parser.add_argument(
        "--batch_size",
        type=int,
    ...
    parser.add_argument(
        "--epochs", type=int, help="number of epochs", default=epochs_default
    )
    parser.add_argument(
        "--optimizer", type=str, help="optimizer", default=optimizer_default
    )
    parser.add_argument(
        "--seed", type=int, help="random seed", default=random_seed_default
    )
    parser.add_argument(
        "--model_dir", type=str, help="model directory", default=model_directory_default
    )
    parser.add_argument(
        "--model_filename",
        type=str,
    ...
    parser.add_argument(
        "--log_dir", type=str, help="log directory", default=log_directory_default
    )
